		...
		dram_execution_list_turn(0), back_pressure_buffer_max_depth(back_pressure_buffer_max_depth)
	{
		...
		switch (sharing_mode)
		{
			case SSD_Components::Cache_Sharing_Mode::SHARED:
			{
				...
				dram_execution_queue = new std::queue<Memory_Transfer_Info*>[1];//the size are one as it is not partitioned
		...
		bloom_filter = new std::set<LPA_type>[stream_count];//bloom filter is different for different streams does not depend whether shared or equally partitioned
	}
				...
				case Caching_Mode::WRITE_READ_CACHE:
				{
					...
					while (it != user_request->Transaction_list.end()) {
						...
						if (per_stream_cache[tr->Stream_id]->Exists(tr->Stream_id, tr->LPA)) {
							#define important_to_note
							#define initiaization_of_available_sectors_bitmap
							page_status_type important_to_note available_sectors_bitmap = per_stream_cache[tr->Stream_id]->Get_slot(tr->Stream_id, tr->LPA).State_bitmap_of_existing_sectors & tr->read_sectors_bitmap;
							...
							#define important_to_note
							#define inititialization_of_read_sectors_bitmap
