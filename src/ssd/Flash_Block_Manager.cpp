...
namespace SSD_Components
{
	...
	void Flash_Block_Manager::Allocate_block_and_page_in_plane_for_RPG_write(const stream_id_type streamID, NVM::FlashMemory::Physical_Page_Address& page_address)
	{
		...
		page_address.PageID = plane_record->Blocks[page_address.BlockID].Current_page_write_index++;
		program_transaction_issued(page_address);
        plane_record->Blocks[page_address.BlockID].Stream_id=streamID;
		...
		if (plane_record->Blocks[page_address.BlockID].Current_page_write_index == pages_no_per_block) {
			...
			plane_record->Blocks[page_address.BlockID] = *(plane_record->Get_a_free_block(streamID, true));
