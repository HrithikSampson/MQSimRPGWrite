...
namespace SSD_Components
{
	...
	{
		if (free_block_pool_size < block_pool_gc_threshold) {// enact gc if no of blocks free is lesser than the threshold
			...
			switch (block_selection_policy) {
				case SSD_Components::GC_Block_Selection_Policy_Type::GREEDY://Find the set of blocks with maximum number of invalid pages and no free pages
				{
					...
					if (pbke->Ongoing_erase_operations.find(0) != pbke->Ongoing_erase_operations.end()) {//checks if blockID 0 can be used for gc
						gc_candidate_block_id++;
					}
					for (flash_block_ID_type block_id = 1; block_id < block_no_per_plane; block_id++) {
						if (pbke->Blocks[block_id].Invalid_page_count > pbke->Blocks[gc_candidate_block_id].Invalid_page_count//if current candidate previous candidate
//																														 block is better than previous presumed 
//																													candidate block
				...
				case SSD_Components::GC_Block_Selection_Policy_Type::RANDOM://randomly selecting a block
				{
