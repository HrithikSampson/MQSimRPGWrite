			...
			for (unsigned int chipID = 0; chipID < chip_no_per_channel; chipID++) {
				...
				for (unsigned int dieID = 0; dieID < die_no_per_chip; dieID++) {
					...
					for (unsigned int planeID = 0; planeID < plane_no_per_die; planeID++) {
						...
						plane_manager[channelID][chipID][dieID][planeID].GC_wf = new Block_Pool_Slot_Type*[total_concurrent_streams_no];//separate gc,data,transaction_
//																																wf for each stream
	...
	bool PlaneBookKeepingType::srch(Block_Pool_Slot_Type* new_block){
	    for(auto i: RPG_Blocks){
	        if(new_block->BlockID==i->BlockID){
	            return true;
	        }
	    }
	    return false;
