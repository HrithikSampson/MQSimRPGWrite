...
namespace SSD_Components
{
	...
	void Data_Cache_Manager_Flash_Simple::write_to_destage_buffer(User_Request* user_request)
	{
		...
		while (it != user_request->Transaction_list.end()
			...
			if (bloom_filter[0].find(tr->LPA) == bloom_filter[0].end()) {	//if it  is in the bloom filter it will be hot data else cold data
		...
		if (Simulator->Time() > next_bloom_filter_reset_milestone) {
			bloom_filter[0].clear();// very important for hot and cold data separation
