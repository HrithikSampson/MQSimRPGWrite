...
namespace SSD_Components {
		...
		: NVM_PHY_ONFI(id, ChannelCount, chip_no_per_channel, DieNoPerChip, PlaneNoPerDie), channels(channels)
	{
		...
		for (unsigned int channelID = 0; channelID < channel_count; channelID++) {
			...
			for (unsigned int chipID = 0; chipID < chip_no_per_channel; chipID++) { 
				bookKeepingTable[channelID][chipID].Expected_command_exec_finish_time = PTRT1; 
				bookKeepingTable[channelID][chipID].Last_transfer_finish_time = PTRT1;
	...
	void NVM_PHY_ONFI_NVDDR2::Setup_triggers()
	{
		...
		for (unsigned int i = 0; i < channel_count; i++) {
			for (unsigned int j = 0; j < chip_no_per_channel; j++) {
				channels[i]->Chips[j]->Connect_to_chip_ready_signal(handle_ready_signal_from_chip);//chips is in onfi_channel_base and Connect_to_chip_ready_signal_from_
																								//Chip(handle_ready_signal_from_cache) is in nvm/Flassh_Chip
			}
		}
	}
	...
	{
		return channels[channelID]->GetStatus();// it is idle/busy
	}
		...
		* then something illegarl is happening*/
		...
		switch (transaction_list.front()->Type) {
			case Transaction_Type::READ:
				if (transaction_list.size() == 1) {//single plane
					Stats::IssuedReadCMD++;
					...
					Stats::IssuedMultiplaneReadCMD++;//multi-plane read to exploit parallelism
				...
				for (std::list<NVM_Transaction_Flash*>::iterator it = transaction_list.begin();
					it != transaction_list.end(); it++) {
					(*it)->STAT_transfer_time += target_channel->ReadCommandTime[transaction_list.size()];//add all read command time in onfi_channel_nvddr2
				}//compute read time
				if (chipBKE->OngoingDieCMDTransfers.size() == 0) {
					targetChip->StartCMDXfer();
					chipBKE->Status = ChipStatus::CMD_IN;

					chipBKE->Last_transfer_finish_time = Simulator->Time() + suspendTime + target_channel->ReadCommandTime[transaction_list.size()];//suspend time in l
																																		//line 146,152,156
				...
				trx_time=targetChip->Get_command_execution_latency(dieBKE->ActiveCommand->CommandCode, dieBKE->ActiveCommand->Address[0].PageID, dieBKE->ActiveCommand->Address[0]);

				dieBKE->Expected_finish_time = chipBKE->Last_transfer_finish_time + trx_time;
			...
			case Transaction_Type::WRITE:
				...
				if (((NVM_Transaction_Flash_WR*)transaction_list.front())->ExecutionMode == WriteExecutionModeType::SIMPLE) {
					...
					} else {//exploitting parallelism using multiplane write
					...
					trx_time=targetChip->Get_command_execution_latency(dieBKE->ActiveCommand->CommandCode, dieBKE->ActiveCommand->Address[0].PageID,dieBKE->ActiveCommand->Address[0]);
					dieBKE->Expected_finish_time = chipBKE->Last_transfer_finish_time + trx_time;
					((Address_Mapping_Unit_Page_Level*)ftl->Address_Mapping_Unit)->send_to_amu(trx_time,dieBKE->ActiveCommand->Address[0],dieBKE->ActiveTransactions.front()->Stream_id);
					if (chipBKE->Expected_command_exec_finish_time < dieBKE->Expected_finish_time) {
						chipBKE->Expected_command_exec_finish_time = dieBKE->Expected_finish_time;
					}
				} else {			//if (((NVM_Transaction_Flash_WR*)transaction_list.front())->ExecutionMode == WriteExecutionModeType::(NOT) SIMPLE) i.e copyback
			...
			case Transaction_Type::ERASE:
				...
				dieBKE->Expected_finish_time = chipBKE->Last_transfer_finish_time +targetChip->Get_command_execution_latency(dieBKE->ActiveCommand->CommandCode, dieBKE->ActiveCommand->Address[0].PageID,dieBKE->ActiveCommand->Address[0]);
				if (chipBKE->Expected_command_exec_finish_time < dieBKE->Expected_finish_time) {
					chipBKE->Expected_command_exec_finish_time = dieBKE->Expected_finish_time;
				}
				break;
	...
	void NVM_PHY_ONFI_NVDDR2::Execute_simulator_event(MQSimEngine::Sim_Event* ev)
	{
		...
		if (WaitingCopybackWrites[channel_id].size() > 0) {
			...
			sim_time_type trx_time=targetChip->Get_command_execution_latency(waitingBKE->ActiveCommand->CommandCode, waitingBKE->ActiveCommand->Address[0].PageID, waitingBKE->ActiveCommand->Address[0]);
	...
	inline void NVM_PHY_ONFI_NVDDR2::handle_ready_signal_from_chip(NVM::FlashMemory::Flash_Chip* chip, NVM::FlashMemory::Flash_Command* command)
	{
		...
		case CMD_READ_PAGE_COPYBACK_MULTIPLANE:
			...
			if (_my_instance->channels[chip->ChannelID]->GetStatus() == BusChannelStatus::IDLE)
			{
				...
				dieBKE->Expected_finish_time = Simulator->Time() + _my_instance->channels[chip->ChannelID]->ProgramCommandTime[dieBKE->ActiveTransactions.size()]
					+ chip->Get_command_execution_latency(dieBKE->ActiveCommand->CommandCode, dieBKE->ActiveCommand->Address[0].PageID, dieBKE->ActiveCommand->Address[0]);
				if (chipBKE->Expected_command_exec_finish_time < dieBKE->Expected_finish_time)
					chipBKE->Expected_command_exec_finish_time = dieBKE->Expected_finish_time;
#if 0	
		...
		{
			DEBUG("2.Chip " << chip->ChannelID << ", " << chip->ChipID << ": finished program command")
			int i = 0;
			for (std::list<NVM_Transaction_Flash*>::iterator it = dieBKE->ActiveTransactions.begin();
	...
	{
		DEBUG("2.Chip " << tr->Address.ChannelID << ", " << tr->Address.ChipID << ": transfer read data started for LPA: " << tr->LPA)
	...
	void NVM_PHY_ONFI_NVDDR2::perform_interleaved_cmd_data_transfer(NVM::FlashMemory::Flash_Chip* chip, DieBookKeepingEntry* bookKeepingEntry)
	{
		...
		switch (bookKeepingEntry->ActiveTransactions.front()->Type)
		{
			case Transaction_Type::READ:
				...
				Simulator->Register_sim_event(Simulator->Time() + bookKeepingEntry->DieInterleavedTime/*fire time*/,
					this/*Sim_object*/, bookKeepingEntry/*parameter*/, (int)NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED/*type*/);
				break;
			case Transaction_Type::WRITE:
