...
#define ENDCODE
/*
	    How to use processvar.h
    	1. Use an object of ProcessVariation class to instantiate PV model into your code.
    ProcessVariation pv;

        2. Call the method:
        vector <vector <int> > getTargetSet (float mu, uint64_t clk_instance, int ndim, int rank_count, int bank_count, int max_banks, bool randomize_param);
    /* pass the following values:
    mu = 0.07;
    clk_instance = _any_random_value;
    ndim = 1;
    rank_count = _total_number_of_planes_;
    bank_count = _total_number_of_blocks_;
    max_banks = _number_of_affected_blocks_;
    randomize_param = true;
    */
...
#define MAKE_TABLE_INDEX(LPN,STREAM)
    ...
    class PageRPGCache{
		    ...
		    std::vector<int> vec;
		    int count;
		    PageRPGCache(std::vector<int> v,int cnt);
		};
	struct CMTSlotType
	{
		PPA_type PPA;
	...
	* its own share of CMT (equal partitioning of CMT space among concurrent streams).*/
	...
	public:
		...
		* proposal. It is always stored in non-volatile flash memory.*/
		...
		NEWCODE
		std::multimap<PPA_type,sim_time_type> pv_aware_time;
		std::map<PPA_type,sim_time_type> start_time;
		ENDCODE
	...
	public:
		...
		void send_to_amu(sim_time_type trx_time,NVM::FlashMemory::Physical_Page_Address& Address,stream_id_type Stream_id);
        ...
        int get_count(int key,int group);
		...
		void print_pv_aware_data(std::string str);
		//std::vector<std::vector<long int>>** v;
		int***** z;
		int*** t;
		double size_of_group_element;
		int group_no=4;//no. of groups in ldpc parity code paper
		int count_size=8;
		bool alloc;
		std::multimap<int,std::vector<int>>** group;
		std::list<std::pair<PageRPGCache*,LPA_type>>* g; 
		LPA_type get_count(LPA_type key,int group);
		PageRPGCache* set1(const stream_id_type,LPA_type);
		std::list<std::pair<PageRPGCache*,LPA_type>>::iterator get_NthChanceBack(int);
		std::map<LPA_type,std::list<std::pair<PageRPGCache*,LPA_type>>::iterator>* map;//this lpa type is to implement lpn to unique stream. stream is the group,lpa is the lpn is the lpa.
		//std::multimap<int,unsigned int> freq_table;
	private:
		static Address_Mapping_Unit_Page_Level* _my_instance;
		unsigned int cmt_capacity;
		AddressMappingDomain** domains;
		void init_v(int channelID,int chipID,std::vector<std::vector<long int>>** v);
		...
		std::map<PPA_type,sim_time_type> pv_update_data();
		...
		PPA_type online_create_entry_for_reads(LPA_type lpa, const stream_id_type stream_id, NVM::FlashMemory::Physical_Page_Address& read_address, uint64_t read_sectors_bitmap,bool RPG_WR);
