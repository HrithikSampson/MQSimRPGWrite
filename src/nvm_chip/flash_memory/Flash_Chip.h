...
namespace NVM
{
    ...
    */
		...
		public:
			...
			sim_time_type Get_command_execution_latency(command_code_type CMDCode, flash_page_ID_type pageID,NVM::FlashMemory::Physical_Page_Address& Address)
			{
				//std::cout<<CMDCode<<std::endl;
				int latencyType = 0;
				...
				switch (CMDCode)
				{
					...
					case CMD_READ_PAGE_COPYBACK_MULTIPLANE:
						...
						return _readLatency[latencyType]*(1/(1+no_of_accesses[Address.DieID][Address.PlaneID][Address.BlockID][Address.PageID]*0.1))  + _RBSignalDelayRead;
					...
					case CMD_PROGRAM_PAGE_COPYBACK_MULTIPLANE:
						...
						return (sim_time_type)(_programLatency[latencyType]*(no_of_accesses[Address.DieID][Address.PlaneID][Address.BlockID][Address.PageID]+1)*(rand()%100+1)+ _RBSignalDelayWrite);
						
						no_of_accesses[Address.DieID][Address.PlaneID][Address.BlockID][Address.PageID]++;
						
					case CMD_ERASE_BLOCK:
					case CMD_ERASE_BLOCK_MULTIPLANE:
					    
						//no_of_READ_accesses[Address.DieID][Address.PlaneID][Address.BlockID][Address.PageID]
...
//A simplification to decrease the complexity of GC execution! The GC unit may need to know the metadata of a page to decide if a page is valid or invalid. 
		private:
			...
			unsigned int**** no_of_accesses;
			unsigned int**** no_of_READ_accesses;
