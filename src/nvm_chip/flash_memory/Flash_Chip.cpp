...
namespace NVM
{
	namespace FlashMemory
	{
		...
		no_of_READ_accesses=new unsigned int***[die_no];
			for(unsigned int die_id=0;die_id<die_no;die_id++)
			{
				no_of_READ_accesses[die_id]=new unsigned int**[plane_no_in_die];
				for(unsigned int plane_id=0;plane_id<plane_no_in_die;plane_id++)
				{
					no_of_READ_accesses[die_id][plane_id]=new unsigned int*[block_no_in_plane];
					for(unsigned int block_id=0;block_id<block_no_in_plane;block_id++)
					{
						no_of_READ_accesses[die_id][plane_id][block_id]=new unsigned int[page_no_per_block];
						for(unsigned int page_id=0;page_id<page_no_per_block;page_id++)
						{
							no_of_READ_accesses[die_id][plane_id][block_id][page_id]=0;
						}
					}
				}
			}
			...
			no_of_accesses=new unsigned int***[die_no];
			for(unsigned int die_id=0;die_id<die_no;die_id++)
			{
				no_of_accesses[die_id]=new unsigned int**[plane_no_in_die];
				for(unsigned int plane_id=0;plane_id<plane_no_in_die;plane_id++)
				{
					no_of_accesses[die_id][plane_id]=new unsigned int*[block_no_in_plane];
					for(unsigned int block_id=0;block_id<block_no_in_plane;block_id++)
					{
						no_of_accesses[die_id][plane_id][block_id]=new unsigned int[page_no_per_block];
						for(unsigned int page_id=0;page_id<page_no_per_block;page_id++)
						{
							no_of_accesses[die_id][plane_id][block_id][page_id]=0;
						}
					}
				}
			}
		...
		Flash_Chip::~Flash_Chip()
		{
			...
			for(unsigned int die_id=0;die_id<die_no;die_id++)
			{
				...
				for(unsigned int plane_id=0;plane_id<plane_no_in_die;plane_id++)
				{
					...
					for(unsigned int block_id=0;block_id<block_no_in_plane;block_id++)
					{
						delete[] no_of_READ_accesses[die_id][plane_id][block_id];
					}
					delete[] no_of_READ_accesses[die_id][plane_id];
				}
				delete[] no_of_READ_accesses[die_id];
			}
			delete[] no_of_READ_accesses;
			for(unsigned int die_id=0;die_id<die_no;die_id++)
			{
				...
				for(unsigned int plane_id=0;plane_id<plane_no_in_die;plane_id++)
				{
					...
					for(unsigned int block_id=0;block_id<block_no_in_plane;block_id++)
					{
						delete[] no_of_accesses[die_id][plane_id][block_id];
					}
					delete[] no_of_accesses[die_id][plane_id];
				}
				delete[] no_of_accesses[die_id];
			}
			delete[] no_of_accesses;
		}
		...
		void Flash_Chip::start_command_execution(Flash_Command* command)
		{
			...
			targetDie->Expected_finish_time = Simulator->Time() + Get_command_execution_latency(command->CommandCode, command->Address[0].PageID,command->Address[0]);
		...
		void Flash_Chip::finish_command_execution(Flash_Command* command)
		{
			...
			targetDie->STAT_TotalReadTime += Get_command_execution_latency(command->CommandCode, command->Address[0].PageID,command->Address[0]);
				...
				case CMD_ERASE_BLOCK_MULTIPLANE:
				{
					for (unsigned int planeCntr = 0; planeCntr < command->Address.size(); planeCntr++) {
						...
						for (unsigned int i = 0; i < page_no_per_block; i++) {
							...
							no_of_READ_accesses[command->Address[planeCntr].DieID][command->Address[planeCntr].PlaneID][command->Address[planeCntr].BlockID][command->Address[planeCntr].PageID]=0;
						}
					}
					break;
				}
